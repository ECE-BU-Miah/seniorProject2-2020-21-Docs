...
#define DEG_PER_MEASUREMENT (STEPS_PER_MEASUREMENT*STEP_MOTOR_DEG_PER_STEP)
#define NUM_MEASUREMENTS (int)(81/DEG_PER_MEASUREMENT)
...
int Addrecord(FILE* fp, xbeeArray_settings* arrayP, double angle, double distance, double height, int numScan);
...
int main(int argc, char* argv[]){
    ...
    // Set up pause button
    MAIN_ASSERT(
        rc_button_init(RC_BTN_PIN_PAUSE, RC_BTN_POLARITY_NORM_HIGH, RC_BTN_DEBOUNCE_DEFAULT_US) == 0,
        "\tERROR: Failed to initalize pause button!!!\n"
    );
    rc_button_set_callbacks(RC_BTN_PIN_PAUSE, __on_pause_press, NULL);

    // Set up mode button for start
    MAIN_ASSERT(
        rc_button_init(RC_BTN_PIN_MODE, RC_BTN_POLARITY_NORM_HIGH, RC_BTN_DEBOUNCE_DEFAULT_US) == 0,
    ...
    if(useSweep){
        ...
        while(rc_get_state() != EXITING && segment < (NUM_MEASUREMENTS + 1)) {
            // Add record
            msleep(100);
            double curAngle = fmod((init_angle + angleOffset), 360.0);
            #ifndef CORE_DISABLE_MESSAGES
            printf("\tCurrent Angle is: %.1f\n", curAngle);
            #endif
            result = Addrecord(fp, &array, curAngle, init_distance, init_height, numScans);
            MAIN_ASSERT(result != -1, " ");

            // Move the stepper motor
            result = stepMotor_Step(&sm, direction, STEPS_PER_MEASUREMENT);
            MAIN_ASSERT(result != -1, " ")
        ...
        // Add record
        result = Addrecord(fp, &array, init_angle, init_distance, init_height, numScans);
        MAIN_ASSERT(result != -1, " ");
    }

    // Close output file
    result = fclose(fp);
    MAIN_ASSERT(result == 0, "\tERROR: Failed to close file.\n");
...
int Addrecord(FILE* fp, xbeeArray_settings* arrayP, double angle, double distance, double height, int scanCount) {
    // Initalize storage variables
    int result;
    ubyte strengths[5];
    ...
    for (int m = 0; m < scanCount; ++m) {
        ...
        // Write out the angle, distance, and height values to the file
        result = fprintf(fp, "%3.2f,%4.3f,%4.3f",angle,distance,height);
        ASSERT(result >= 0, "\tERROR: Failed to write to the file\n");
