...
Robotic carts are a prevalent invention designed to aid \replace{the
  customers}{users} in a number of important indoor and outdoor tasks. There are
\add{robotic} systems currently \add{available} \replace{on}{in} the market that
...
second cart to carry more items. \add{Moreover, robots used for this purpose are
more costly than they worth.} In this project, we are proposing a robotic
...
the customer through the store. \replace{This}{The} implementation of \add{such}
...
field. Applications of \replace{this system}{the proposed robotic cart} include
\add{but not limited to} \comment{mail delivery carts}{need to clarify this application}, file
...
Abundant research in the field of \replace{robotic carts}{mobile robotics} shows
various ways \replace{in which to
implement a system}{to develop robotic carts} that will help consumers \replace{with}{in}
carrying groceries through stores\add{.
  See~\cite{Rawashdeh2017-Person,islam_lam_fukuda_kobayashi_kuno_2019,Sales2016-CompaRob}, 
for example}. 
...
shopping habits of customers while also using a LiDAR\add{ (Light Detection and Ranging)} sensor and camera to
detect and map the customer using a two-dimensional skeleton and follow them
through the store~\cite{islam_lam_fukuda_kobayashi_kuno_2019}. \replace{In the last
method}{Therein}, \remove{the }authors utilized an Arduino MEGA 2560, six ultrasonic sensors, two
DC motors with PWM (Pulse Width Modulation), an Android Studio IDE device, and
Bluetooth \replace{in order to}{for} detect\add{ing} \replace{the}{a} customer and follow them through the
store~\cite{Rawashdeh2017-Person}. In \replace{our}{the current} project, we are implementing XBee S2C
RF radios, which are inexpensive and easily configurable, as a
\comment{remote}{Define remote} that the
...
at various distances and angles\add{, \textit{i.e.,} angle of arrivals of RF
  signals from the remote}, based on research done previously in this type
...
common in Wi-Fi based wireless positioning sensing systems. \replace{Some
  methods, seen}{Authors} 
...
in received signals. \add{The work in }~\cite{ladd_bekris_rudys_kavraki_wallach_2005} shows a
...
There are three \add{primary} requirements for our \replace{system}{proposed
  mobile robotic cart}. \add{These requirements are stated as follows.}

\begin{enumerate}
\item \add{The mobile cart to follow the remote attached to the user while
    maintaining a safe distance of $1~[\meter]$ to $1.5~[\meter]$}. % 
%
  \remove{The first requirement is that the cart should be able to follow the remote while maintaining a distance of $1$ to $1.5$ meters.} This requirement
  \replace{comes}{stems} from the intended usage of the project, which is a cart
...
\item   The second requirement for the system is that the cart should be able to travel at \replace{$1$ m/s}{$1~[\si{\meter\per\second}]$}. If the cart can only travel slowly, it will not be as useful since the user will have to walk very slowly to allow the cart to keep up.

\item   \replace{The third requirement is that line-of-sight communication
    between the cart and the robot must not be required.}{The proposed cart is
    not to rely on line-of-sight communication between the remote (user) and the
  cart.} In the case that an obstacle gets between the cart and the remote, the communication should be able to be maintained so that the cart still knows where the user is located.
\end{enumerate}


\section{System Architecture}
The \add{overall} system \add{architecture} \replace{for}{of} this project
consists of two subsystems which are the Mobile Cart and the Remote Target
\add{(user or customer, for example)}\replace{. as seen}{ as shown} in Figure
\ref{fig:sys_block_diag}. The \replace{Mobile Cart system}{proposed mobile cart}
is a wheeled robot that sends and receives radio signals to follow the remote
target. The \replace{Remote Target}{remote} acts as the target for the
\replace{Mobile Cart in the system}{mobile cart system} by sending radio signals back and forth with the cart.

\subsection{System Block Diagram}
The \add{high-level system} block diagram of the \replace{overall
  system}{proposed robotic cart (prototype)} is shown in
\autoref{fig:sys_block_diag}. There are four inputs to the \add{proposed cart}
system. \add{Obviously,} \replace{the system must have a power source, which
  will be a battery}{the robotic cart is powered on through a source of power
  (input),  which is simply a battery, for instance}. There will
...
different operating modes \add{which will be explained later on}.

\vspace*{12pt}
\noindent
The main output of the system is the trajectory of the cart \add{on the ground}.
As the user moves with the remote, the cart \replace{will}{is supposed to} move
\remove{to} follow the user. The other output of the robotic cart system is status indication in the form of LED lights. These lights will be used to notify the user of the status of the system.
...
will be used to directionalize the \add{RF (analog)} signals \replace{coming
  from}{transmitted by } the remote\add{, which is attached to a user, for instance}.
...
\vspace*{12pt}
\noindent
Two reflector designs \add{for the direction-sensitive receiver mounted on the
  mobile cart} will be evaluated in this project. The first, shown in
...
\todo[inline]{Replace the subsection title with ``Operation of Mobile Cart
  System''} The control of the Mobile Cart will be handled by a central embedded
computer\replace{, which will be a BeagleBone Blue}{BeagleBone Blue, for instance}. Two DC motors will be used to drive
...
\begin{figure}
  \centering
  \input{figs/dia/algorithmFlowchart.tex}
