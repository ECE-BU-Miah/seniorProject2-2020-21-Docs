...
\begin{figure}[htbp]
  \begin{subfigure}[b]{0.9\linewidth}
    ...
    \begin{tikzpicture}    
      ...
      \begin{pgfonlayer}{background}
        ...
        % %%% Inputs and outputs
        \draw[stealth-,thick]
        ($(tmp1)+(0,2.0*\smgrid)$) --node[near end,above]{Battery power}++(-3.0*\smgrid,0);
        \draw[stealth-,thick]
        ($(tmp1)+(0,0.75*\smgrid)$) --node[very near end,above]{Incoming RF messages}++(-3.5*\smgrid,0);
        \draw[-stealth,thick]
        ($(tmp2)+(0,-1.25*\smgrid)$) --node[midway,above]{Outgoing RF messages}++(5.25*\smgrid,0);
      \end{pgfonlayer}
    \end{tikzpicture}  
    \caption{}
  ...
  \begin{subfigure}[b]{0.9\linewidth}
    ...
    \begin{tikzpicture}[scale=1.15,transform shape]    
      ...
      \node[block,text width= 1.5cm, text centered, below of=multiplexer, node distance=0.75cm](RFReceiver1){Direction sensitive RF receiver};
      \node[block,text width= 1.5cm, text centered, below of=RFReceiver1, node distance=0.75cm](RFReceiver2){Direction sensitive RF receiver};
