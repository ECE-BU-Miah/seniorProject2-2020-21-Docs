...
\vspace*{12pt}
KA\\
Today I worked on the draft outline of the final report and created the chapters in Github for us to edit later. We divided the chapters among the group and each of us will work on two chapters.

\labday{Thursday, April 8, 2021}
\experiment{Lab Time}
KA\\
Today we worked on finishing up the slides for the final presentation since the draft is due today. Jason and I also tested some code for tuning the distance estimation using the encoders on the wheel motors.

\experiment{Meeting Agenda}
JB\\
\begin{itemize}
    \item Final Report and Presentation Drafts
    \item IAB Poster presentation
\end{itemize}

\experiment{Meeting Minutes}
JB\\
In the meeting today, we discussed the state of the project, which is not working properly. We also discussed the final report and presentation drafts. Dr. Miah told us to not spend much time on the drafts, but instead to focus on getting the robot to work. Finally, we discussed the IAB poster presentation. Dr. Shastry informed us that we are not going to have a poster presentation, but instead will submit a 10-minute video to the IAB.

\labday{Tuesday, April 13, 2021}
\experiment{Lab Time}
JB\\
Today Kalli and I collected some signal strength data with the robot. Then I worked on training a neural network with tensorflow to be able to predict the angle based on the relative signal strengths. After the normal lab time, I worked on implementing the neural network in C code. I tried testing the code, but I ran into a problem with the XBees where they would not communicate properly.

\vspace*{12pt}
KA\\
In the lab today Jason put the model 2 reflector array on the robot since we determined that it will give us better results. Jason and I tested the robot a few times and decided to test out a neural network to se if it will improve the robots angle estimation and distance calculation. I also worked on editing the draft of the final paper based on Dr. Shastry's comments.

\labday{Thursday, April 15, 2021}
\experiment{Lab Time}
JB\\
Today in the lab I found that the XBee error seemed to be coming because the remote XBee stopped working. I replaced it with a different XBee, but it still did not work. I found that the PAN ID was not configured properly on the new XBee. I reconfigured that, and the XBee communication worked again. After that, Kalli and I tested the neural network program. The neural network was very poor at estimating the angle. We decided to go back to our previous approach, so we ran that program again. For some reason, it worked much better this time than it ever has before. We decided to try it in the atrium of the BECC to have more open space. It works fairly well in the open space. When there are objects closer, such as in the lab, it still somewhat works, but not as well as in the open. Kalli recorded some videos of the robot in the atrium.

\vspace*{12pt}
KA\\
After working with the neural network in yesterday's lab we noticed that we were not getting consistent enough results so we decided to go back to an older version of the original code. We were able to get better accuracy in the robot's trajectory to the remote with the older version of the code. After doing a few tests we decided to go to the first floor of BECC and record videos for our presentation. We got two videos where we ran the robot at a normal speed then at a higher speed.

\experiment{Meeting Agenda}
JB\\
Show videos of progress

\experiment{Meeting Minutes}
JB\\
In today's meeting, we showed our videos of the robot following the remote to Dr. Miah and Dr. Shastry. The meeting was then ended early.

\labday{Tuesday, April 20, 2021}
\experiment{Lab Time}
JB\\
Today I worked on updating the code to calculate the duty cycles properly instead of using the patch method we had previously. Then Kalli and I tested it in the hallway. It worked somewhat. We also tried using some of the RF absorbers from the RF lab. They did seem to help, which indicates that at least some of the problem we are having with incorrect angle calculation is due to the reverberations from the environment.

\vspace*{12pt}
KA\\
Today we worked on finalizing the project and tested/corrected code to see if we could get any more progress. We also worked on finalizing the presentation slides for the IAB video and for the final presentation for ECE 499. We did one last test with the robot with using the absorbers in front of the cabinets in the lab. This helped the robot avoid the cabinets since it had a problem with running into them since the reflective coating of cabinets would reflect a stronger signal than the remote.

\labday{Thursday, April 22, 2021}
\experiment{Lab Time}
JB\\
Today we collected several videos of our project running in various places. We took some outside, some in the basement of the BECC, some in the hallway, some in the atrium, and some in the senior lab. These videos will be used as the demo part of the IAB video, as well as in the final presentation next Tuesday.

\vspace*{12pt}
KA\\
Today Jason and I went to different areas of BECC to film the robot running in different terrain. We will be using these videos for the IAB video and for our final presentation.

\experiment{Meeting Agenda}
JB\\
\begin{itemize}
    \item Discuss IAB video
    \item Discuss Final Presentation
    \item Discuss Final Report
\end{itemize}

\experiment{Meeting Minutes}
JB\\
Today we first discussed the IAB video. Dr. Miah told us that if it is slightly longer than 10 minutes, that is ok, but we should try to keep it around 10 minutes. He also told us to have a lot of demo videos. Next we discussed the final presentation, which will be Tuesday, April 27, from 2:30 - 3:00 pm. We will try to get the slides done by tomorrow (Friday) so that we can send them to Dr. Miah and Dr. Shastry for feedback and still have time to make edits before the actual presentation. Finally, we discussed the final report, which is due on May 4th. We are planning to try to have it done by April 30th so that we can get feedback and make edits if needed.
